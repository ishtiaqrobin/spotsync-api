# SpotSync – Project Concepts & Keywords
...
**Why:** Enforces encapsulation. If this project were a library, consumers couldn't accidentally use our internal plumbing. Only code inside `spotsync-api/` can import `spotsync-api/internal/...`.

```
internal/
├── auth/           ← JWT service interface + implementation
├── config/         ← env + DB setup
├── middleware/     ← HTTP middleware (auth + role)
...
- What this module is called (`module github.com/ishtiaqrobin/spotsync-api`)
- Which Go version to use (`go 1.26.4`)
- What external packages are needed (`require` block)

```go
module github.com/ishtiaqrobin/spotsync-api
...
os.Getenv("DSN")          // reads individual values
```
...
**Why:** Some files must never go to GitHub — secrets (`.env`), compiled binaries (`*.exe`), build output (`tmp/`), and local tool settings (`.vscode/`).
...
**Why:** Represents real-world objects — a User, a ParkingZone, a Reservation.
...
**Why:** Decouples code. A service doesn't need to know _how_ the repository works — only _what_ it can do.
...
| `json:"-"`                      | `encoding/json`           | Field is never included in JSON output (used for Password) |
...
| `validate:"oneof=driver admin"` | `go-playground/validator` | Value must be one of the listed options                    |
...
This project uses **layered architecture** inside each domain. Every domain (`user`, `zone`, `reservation`) has the same 5 layers:
...
**What:** One feature area. `user` handles auth. `zone` handles parking zones. `reservation` handles bookings.
...
**What:** Structs used to carry data _into_ the API (request) or _out of_ the API (response). Separate from the entity.
...
```go
func (s *service) Login(req dto.LoginRequest) (*dto.LoginResponse, error) {
    user, _ := s.repo.FindByEmail(req.Email)  // ask repository
    if user == nil {
...
func (h *handler) Login(c echo.Context) error {
    ...
    response, err := h.service.Login(req)  // 3. call service
...
**What:** A function that runs _before_ a handler. Can inspect/modify the request or block it entirely.
...
**Why:** Keeps `server/http.go` clean. It just calls `user.RegisterRoutes(...)`, `zone.RegisterRoutes(...)`, etc.
...
func RegisterRoutes(e *echo.Echo, db *gorm.DB, cfg *config.Config) {
    ...
    api.POST("/register", handler.Register)   // attach handler to route
...
**What:** All API responses follow a standardized format using `httpresponse` package.

**Why:** Consistent response structure makes frontend integration easier and debugging simpler.

```go
// Success response
httpresponse.Success(c, 201, "User registered successfully", response)
// Output: { "success": true, "message": "...", "data": {...} }

// Error response
httpresponse.Fail(c, 400, "Validation failed", validationErrors)
...
**What:** A method you can define on a struct to explicitly set the database table name.

**Why:** By default, GORM pluralizes the struct name to get the table name (e.g., `User` → `users`, `ParkingZone` → `parking_zones`). But when you have a struct that's just a reference to another table (like `reservation.Zone` which points to `parking_zones`), you need to explicitly set the table name.

```go
// internal/domain/reservation/entity.go
type Zone struct {
    gorm.Model
    Name          string
    Type          string
...
// Explicitly set table name to avoid GORM creating a separate "zones" table
func (Zone) TableName() string {
    return "parking_zones"
}
```

Without this, GORM would create a `zones` table (empty) and the foreign key would point to the wrong table.
...
Middleware (if route is protected — checks JWT token)
    │  valid token → sets user_id, user_role in context
...
→ if valid: sets user_id, user_role in context, calls handler
...
| `errors`  | `string/object/null` | Error details                |
